...
Three AAOSA agents run **on top of mimOE**, deployed as edge microservices (mims) and served from each node's mimOE gateway on `:8083`. Inference is mimik ai's OpenAI-compatible endpoint on one node. There are no standalone Node processes on any node. Every node builds its own addon from source, so Node.js 18+ and npm are needed on each of them; Node is the only runtime involved.
...
Node A normally hosts inference for the whole mesh, but inference is per node: any node can point `INFERENCE_URL` at its own local model instead.
...
One image serves all three roles. The node's `.ini` decides `NAME`, `AGENT_KIND` (`frontman` | `specialist` | `device`), `DESCRIPTION`, and `INFERENCE_URL`. Routing quality is description quality: the `DESCRIPTION` string is the only thing peers see when deciding whether an inquiry belongs to that agent.
...
4. **The punchline: add a brand-new agent mid-run.** Install the addon on a fourth node with a new `NAME` and `DESCRIPTION`, for example a storage agent. Its mimOE advertises the service and it joins the consult set on the next inquiry. Compare with the static-configuration world, where new membership is a config change and a redeploy: here membership is runtime mesh state.
...
- A peer's entire `/aaosa` reply, including its own inference call, must return inside mimOE's outbound-read ceiling of roughly 20 s. A slow local model on a peer blows through it and the coordinator records `unreachable`. This is why the `network` role ships with a brevity instruction and a 256-token cap by default.
