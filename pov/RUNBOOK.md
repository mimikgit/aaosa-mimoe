...
1. The three routing scenarios:
   ...
   The first should route to the device agent alone, the second to the network agent alone, the third to both with a synthesized answer.
...
2. The dynamism showcase:
       ...
       bash pov/dynamic-demo.sh          # reads MBP1_IP from env.sh
...
- **`no built addon in mim/build/`**: `remote.sh` copies a built addon rather than building on each node. Run `bash mim/build.sh && bash mim/package-addon.sh` on this machine first (steps A4 and A5).
